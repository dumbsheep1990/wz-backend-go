...
import (
	...
	pb "wz-backend-go/api/component"
...
func main() {
	...
	// Assemble the component service
	componentAssembly := assembly.NewComponentServiceAssembly(db, txManager)
	...
	pb.RegisterComponentServiceServer(grpcServer, componentAssembly.GRPCAdapter)
	
	// Handle graceful shutdown
	go func() {
		log.Printf("Starting component gRPC server on port %s", port)
	...
	log.Println("Shutting down component gRPC server...")
	grpcServer.GracefulStop()
	log.Println("Component server exited properly")
}
