...
func main() {
	...
	// Assemble the order service
	orderAssembly := assembly.NewOrderServiceAssembly(db, txManager)
	...
	pb.RegisterOrderServiceServer(grpcServer, orderAssembly.GRPCAdapter)
	
	// Handle graceful shutdown
	go func() {
		log.Printf("Starting order gRPC server on port %s", port)
	...
	log.Println("Order server exited properly")
}
