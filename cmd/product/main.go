...
func main() {
	...
	// Assemble the product service
	productAssembly := assembly.NewProductServiceAssembly(db, txManager)
	...
	pb.RegisterProductServiceServer(grpcServer, productAssembly.GRPCAdapter)
	
	// Handle graceful shutdown
	go func() {
		log.Printf("Starting product gRPC server on port %s", port)
	...
	log.Println("Shutting down product gRPC server...")
	grpcServer.GracefulStop()
	log.Println("Product server exited properly")
}
