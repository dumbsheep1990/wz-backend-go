...
import (
	...
	"wz-backend-go/internal/delivery/rpc/component"
...
var configFile = flag.String("f", "etc/component.yaml", "the config file")
...
func main() {
	...
	s := zrpc.MustNewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		component.RegisterComponentServiceServer(grpcServer, server.NewComponentServiceServer(ctx))
	...
	// 启动服务
	fmt.Printf("Starting component rpc server at %s...\n", c.ListenOn)
	go s.Start()
	
	// 等待终止信号
	<-quit
	fmt.Println("正在关闭组件服务...")
	s.Stop()
	fmt.Println("组件服务已安全关闭")
}
