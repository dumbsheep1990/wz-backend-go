...
import (
	...
	"wz-backend-go/internal/delivery/rpc/site"
...
var configFile = flag.String("f", "etc/site.yaml", "the config file")
...
func main() {
	...
	s := zrpc.MustNewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		site.RegisterSiteServiceServer(grpcServer, server.NewSiteServiceServer(ctx))
	...
	// 启动服务
	fmt.Printf("Starting site rpc server at %s...\n", c.ListenOn)
	go s.Start()
	
	// 等待终止信号
	<-quit
	fmt.Println("正在关闭站点服务...")
	s.Stop()
	fmt.Println("站点服务已安全关闭")
}
