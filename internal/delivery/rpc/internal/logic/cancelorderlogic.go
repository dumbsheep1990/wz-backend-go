...
func (l *CancelOrderLogic) CancelOrder(in *trade.CancelOrderRequest) (*trade.CancelOrderResponse, error) {
	...
	if err != nil {
		l.Error("Failed to cancel order", logx.Field("error", err), logx.Field("orderId", in.OrderId))
		return &trade.CancelOrderResponse{
			Success: false,
			Error:   err.Error(),
		}, nil
	}
	...
	return &trade.CancelOrderResponse{
		...
		Order: &trade.Order{
			...
			CancelledAt:   orderDTO.CancelledAt.Unix(),
		},
	}, nil
}
