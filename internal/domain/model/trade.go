...
type Product struct {
	...
	MinOrder      int32     `json:"min_order,omitempty" db:"min_order"`
...
type ProductRepository interface {
	// GetByID 根据ID获取产品
	GetByID(productID int64) (*Product, error)

	// List 产品列表
	List(companyID int64, category, keyword string, priceMin, priceMax float64,
		sortBy, sortOrder string, page, pageSize int) ([]*Product, int, error)

	// GetRelated 获取相关产品
	GetRelated(productID int64, limit int) ([]*Product, error)

	// UpdateViews 更新浏览量
	UpdateViews(productID int64) error
}

// ProductService 产品服务接口
type ProductService interface {
	// GetProduct 获取产品详情
	GetProduct(productID int64) (*Product, error)

	// ListProducts 获取产品列表
	ListProducts(companyID int64, category, keyword string, priceMin, priceMax float64,
		sortBy, sortOrder string, page, pageSize int) ([]*Product, int, error)

	// GetRelatedProducts 获取相关产品
	GetRelatedProducts(productID int64, limit int) ([]*Product, error)
}
